          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <div v-for="item in navigation" :key="item.name">
                ...
                <div v-else>
                  <NuxtLink :to="item.href" :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-900 hover:bg-gray-50', 'inline-flex w-full justify-center gap-x-1.5 rounded-md px-3 py-2 text-sm']" :aria-current="item.current ? 'page' : undefined">
                    {{ item.name }}
                  </NuxtLink>
                </div>
